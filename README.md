...
**Design For Manufacturability**の略。半導体設計におけるDFMは、製造されたチップの歩留まり率(Yield)を上げる為に行われる設計段階での工夫のことを総称する。DFMツールは、メタル挿入(CMPでのDishing対策）、ダブルVIA挿入(VIAでのオープン対策)、配線拡張（配線切れ対策）など、配線に関連するDFM機能が重要であるが、リソグラフィーの解像度を改善する(OPC=Optical Proximity Correction：光学近接効果補正)為の補助パターン挿入ツールも広義のDFMツールである。またメモリの冗長回路、ECC回路の採用や、カスタムレイアウト設計で最小寸法をなるべく避ける等も広義のDFMに含まれる。
